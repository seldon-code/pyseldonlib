...
def test_activity_driven():
    ...
    initial_network_settings = pd.InitialNetworkSettings(number_of_agents = 1000,
                                                          connections_per_agent = 10)
    output_settings = pd.OutputSettings(n_output_agents = 1,
                                          print_progress = True # Print the iteration time ; if not set, then does not print
                                          )
    
    options = pd.SimulationOptions(model_string = "ActivityDriven",rng_seed = 120, output_settings = output_settings, model_settings = settings, network_settings = initial_network_settings)
