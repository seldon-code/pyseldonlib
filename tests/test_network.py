...
def network_class_tests():
    ...
    network = pd.seldoncore.generate_n_connections(n_agents = n_agents, n_connections = n_connections,self_interaction = self_interaction, gen = gen_pseudorandom)
    ...
    neighbour_list = [0, 10]
    weights = [0.5, 0.5]
    network.set_neighbours_and_weights(agent_index=agent_index, buffer_neighbours  =neighbour_list, buffer_weights=weights)
    ...
    for i_agent in range(network.n_agents()):
        assert network.get_neighbours(i_agent) == neighbour_no_double_counting[i_agent], f"Neighbours mismatch for agent {i_agent}"
        assert network.get_weights(i_agent) == weights_no_double_counting[i_agent], f"Weights mismatch for agent {i_agent}"
    ...
    for i_agent in range(network.n_agents()):
        assert network.get_neighbours(i_agent) == desired_neighbour_list[i_agent], f"Neighbours mismatch for agent {i_agent}"
